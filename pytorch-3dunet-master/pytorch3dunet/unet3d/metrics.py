...
class SegmentationMetrics:
    """
    Накапливает confusion matrix между вызовами __call__.
    Каждый вызов __call__ возвращает метрики, посчитанные ГЛОБАЛЬНО
    по всем данным, накопленным с последнего reset() (а не по одному батчу).

    Вызывайте reset() в начале каждой эпохи (train и val отдельно!),
    ...
    def _update_confusion_matrix(self, pred, target):
        ...
        idx = self.n_classes * target[mask].astype(int) + pred[mask].astype(int)
        cm = np.bincount(idx, minlength=self.n_classes ** 2).reshape(self.n_classes, self.n_classes)
    ...
    def __call__(self, output, target):
        ...
        # накапливаем текущий батч в общую confusion matrix
        self._update_confusion_matrix(pred, target)

        # считаем метрики по ВСЕМ накопленным с начала эпохи данным
