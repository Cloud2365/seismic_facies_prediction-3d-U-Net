...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--checkpoint", default="./my_models/best_checkpoint.pytorch")
...
def main() -> None:
    ...
    patch_cm = np.zeros((n_classes, n_classes), dtype=np.int64)
        ...
        for input_tensor, indices in tqdm(loader, desc="Testing", unit="batch"):
            ...
            for i, index in enumerate(indices):
                ...
                if gt_patch.shape != pred_patch.shape:
                    raise RuntimeError(
                        f"GT patch {gt_patch.shape} != pred {pred_patch.shape} at {sl}"
                    )

                # Накопление patch-level confusion matrix сразу для текущего патча.
                # Все voxels не сохраняются в огромных списках.
    ...
    # Patch-level confusion matrix уже была накоплена во время inference.
